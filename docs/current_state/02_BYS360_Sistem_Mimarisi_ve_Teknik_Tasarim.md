...
Sonuç: Redis'in "senkron/çevrimdışı yedek çalışma modu"na sahip olduğu iddiası — önceki belge setinde `DOCUMENTATION_DERIVED` olarak işaretliydi — bu incelemeyle **`CODE_VERIFIED` durumuna yükseltilmiştir** — Redis mimari olarak isteğe bağlıdır, yoksa sistem dosya/bellek tabanlı yedek moda düşer, açılışı engellemez.
...
3. `@login_required` / rol tabanlı dekoratörler (`app/security/`, `app/route_support.py`, `app/auth/decorators.py`) yetki kontrolünü uygular (ayrıntılı yetkilendirme mimarisi DOC-05/11 kapsamındadır).
...
- **Kimlik/organizasyon/yetki çekirdeği:** `app/auth/`, `app/security/`, `app/models/core_models.py`, `app/models/org_models.py` — kullanıcı, organizasyon birimi, rol/yetki modelleri. Derinlemesine yetkilendirme mimarisi Belge 05 ve Belge 11 kapsamındadır; bu belge yalnızca konumu belirtir.
...
`scripts/release/build_bys360_safe_release.py`, kaynak kod bütünlüğünü SHA256 ile doğrulayan, `RELEASE_SOURCE_SHA` gömülü bir paket üretir (`CODE_VERIFIED` — dosya mevcut). Bu paket, aday hazırlama (§10) sürecinin girdisidir. Ayrıntılı release/rollback prosedürü Belge 14 kapsamındadır.

---

## Ek — Bu belgede tespit edilen açık teknik gözlemler
