...
**Kanıt kaynağı:** `git log --oneline` (bu incelemede doğrudan çalıştırıldı) — CODE_VERIFIED.
...
| **AL** | **Çapraz veritabanı metadata-sorgu ailesi** | **AÇIK** — bu HEAD'de kapanış commiti bulunamadı. Bu durum BYS360 geliştiricisi tarafından da doğrulanmıştır: AL ve sonraki teknik defter hâlâ açıktır. |

**ID / CATEGORY:** AL / Cross-database metadata-query family
**DESCRIPTION:** AJ (çapraz veritabanı şema) ve AK (Phase7 veritabanı introspeksiyonu) ile aynı ailede, henüz kapatılmamış bir sonraki madde. Bu incelemede AL'nin tam teknik tanımı repository içinde bağımsız olarak bulunamadı (AJ/AK'nin kapanış commit gövdeleri incelenmedi — bu, kapsam dışı bir kaynak kod/geçmiş derin incelemesi gerektirir; bu fazın kapsamı AL'nin düzeltilmesini içermemektedir).
...
**DESCRIPTION:** Registry kendi içinde 0 aktif madde bildiriyor; ancak bu current-state fazı bu iddiayı bağımsız olarak yeniden koşmadı (bu fazın kapsamı tam test paketinin yeniden koşulmasını içermez).
**CURRENT STATUS:** Registry beyanına göre KAPALI (0 aktif) — DOCUMENTATION_DERIVED, bu incelemede yeniden doğrulanmadı.
...
## 3. Önceki Kayıtlarda Adı Geçen, Bu İncelemede Repository İçinde Birebir Doğrulanamayan Maddeler

Aşağıdaki maddeler, bu current-state fazının başında iletilen bilgiler arasında açıkça adlandırılmıştır. Bu inceleme, bu isimlerle **birebir eşleşen** bir kayıt/rapor bulamamıştır (repository genelinde `grep` ile arandı). Bu, maddenin var olmadığı anlamına **gelmez** — yalnızca bu incelemenin onu tam ifadesiyle mekanik olarak konumlandıramadığı anlamına gelir. Madde icat edilmez ilkesi gereği, bu maddeler **silinmez, ancak doğrulanmamış olarak işaretlenir.**

| ID / CATEGORY | Açıklama (önceki kayıttan) | Durum | Kanıt |
|---|---|---|---|
| Dashboard LOW | Dashboard modülünde düşük öncelikli bilinen madde | NOT_YET_FINALIZED — repository taramasında birebir bulunamadı | PRODUCTION_HISTORICAL (önceki kayıt) |
| M drift risk | Bilinmeyen bir "M" etiketli sürüklenme (drift) riski | NOT_YET_FINALIZED — repository taramasında birebir bulunamadı | PRODUCTION_HISTORICAL (önceki kayıt) |
...
| Dosya Merkezi izin (permission) sürüklenmesi | Dosya Merkezi'nin 14 ayrı yetki bayrağı modeli ile ilgili olası tutarsızlık | NOT_YET_FINALIZED — repository taramasında birebir bulunamadı | PRODUCTION_HISTORICAL (önceki kayıt) |
| Dosya Merkezi audit/rollback maddeleri | Dosya Merkezi denetim kaydı veya geri alma akışıyla ilgili olası eksik | NOT_YET_FINALIZED — repository taramasında birebir bulunamadı | PRODUCTION_HISTORICAL (önceki kayıt) |
| PostgreSQL ROUND uyarısı | PostgreSQL `ROUND()` fonksiyonu ile ilgili bir tip/uyumluluk uyarısı | NOT_YET_FINALIZED — `app` altında `round(` çağrıları 15 dosyada bulundu (ör. `app/services/performance/common.py`, `app/api/mobile/domains/kpi_target_management.py`) ancak spesifik bir "PostgreSQL ROUND uyarısı" kaydı/log'u bu incelemede bulunamadı | CODE_VERIFIED (yalnızca `round(` kullanım yerleri) + PRODUCTION_HISTORICAL (önceki kayıt) |
| PT72H / RestartCount / AllowHardTerminate | Scheduled Task sertleştirme maddesi | **UYGULANMIŞ — canlı görevde** (`ExecutionTimeLimit=PT0S`, `RestartCount=3`, `RestartInterval=PT1M`, mekanik olarak doğrulandı). Repo'daki kurulum script'i bu parametreleri hâlâ geçirmiyor — görev script'ten yeniden kaydedilirse sertleştirme kaybolur; bu, ayrı ve düşük öncelikli açık bir takip maddesidir (bkz. DOC-13 §5, §11). | SCRIPT_VERIFIED (negatif grep, script kaynağı) + canlı görev üzerinde mekanik doğrulama (bkz. DOC-13) |

**Not:** Final teknik kapanış dalgasında bu maddelerin her biri için kurum/geliştirme ekibiyle birlikte kaynak (hangi rapor, hangi tarih, hangi kişi bu maddeyi tanımladı) netleştirilmeli; bu current-state fazı bunu icat etmemiştir.

---

## 4. Operasyonel Gözlem — Genel Kod Sağlığı Bağlamı (bilgi amaçlı, defekt değil)

`reports/quality/BYS360_OPS_AUDIT_CI.md` (2026-08-23 üretim zamanlı, izlenmeyen/committed olmayan bir rapor): 976 Python dosyası, 0 syntax hatası, 2101 adet `except Exception` kullanımı (genel bir hata-yönetimi kalıbı, tek seferde "temizlenmemesi", dosya dosya ve davranış bozmadan ele alınması gerektiği raporun kendi notu), 35 potansiyel secret dosyası / 81 potansiyel secret eşleşmesi (bu current-state fazı bunları doğrulamadı — DOC-05/DOC-09'daki güncel secret-gate bulgularıyla çapraz okunmalı).

---

## Özet Tablo

| Kategori | Açık madde sayısı (bu incelemede doğrulanan) | Sonraki adım |
|---|---|---|
...
| Önceki kayıtlarda adı geçen, birebir konumlanamayan maddeler | 6 (Dashboard LOW, M drift, mobil fail-open/drift, Dosya Merkezi izin sürüklenmesi, Dosya Merkezi audit/rollback, PostgreSQL ROUND) | Kurum ile kaynak netleştirmesi + final kapanış dalgasında araştırma |
| PT72H/RestartCount/AllowHardTerminate | 0 (canlı görevde uygulanmış ve mekanik doğrulanmış) — yalnızca script/canlı-görev uyumsuzluğu açık | Repo kurulum script'inin `ExecutionTimeLimit`/`RestartCount`/`RestartInterval` parametrelerini geçirecek şekilde güncellenmesi (bkz. DOC-13 §5, §11) |
