...
Sürüm: Current-State 1.0 (v2 — dokümantasyon sertleştirme fazı, DOC-20 eklendi + çapraz doğrulama bulguları işlendi)
Tarih: 2026-09-02
...
`docs/current_state/` altındaki tüm belgeler (19 belge + facts ledger, DOC-20 dahil) tam metin olarak okunmuş, aşağıdaki kanonik olguların her belgede tutarlı biçimde ifade edildiği doğrulanmıştır. Bu v2 güncellemesinde, ayrıca belgeler arasında **sistematik bir çapraz inceleme** yürütülmüştür — bkz. "Çapraz inceleme bulguları" bölümü. Bir hücre **UYUMLU** ise, ilgili belge bu olguyu ya doğrudan belirtir ya da hiç değinmez (çelişki yoktur, sessizlik çelişki sayılmaz).

## Bulunan ve düzeltilen çelişkiler

**1. (Önceki fazda tespit edildi)** DOC-10 §2 ve DOC-16 §4, `postgresql-x64-15` Windows servis-adı kontrolünün `prepare_bys360_candidate.ps1`/`cutover_bys360_candidate.ps1` içinde var olduğunu iddia ediyordu. DOC-03 §3 ise bunu aramış ve bulamamıştı. `grep` ile doğrulanmıştır: **DOC-03 doğrudur**, bu kontrol yalnızca eski/tarihsel `deploy_bys360_ec4e56b_production_v*.ps1` script'lerindedir. DOC-10/DOC-16 düzeltilmiştir. **ÇÖZÜLDÜ.**

**2-4. (Çapraz inceleme, bu faz)** Bkz. aşağıdaki "Çapraz inceleme bulguları" bölümü — DOC-02/DOC-12 belgelerinde tespit edilen 3 madde (1 FACTUAL_CONTRADICTION, 1 BROKEN_REFERENCE, 1 TERMINOLOGY_DRIFT), tümü aynı belgelerde düzeltilmiştir. **ÇÖZÜLDÜ.**
...
## Çapraz inceleme bulguları (bu fazda yürütüldü)

**Yöntem:** Belge seti üç teknik alana ayrılmıştı: Mimari/Veri/Modüller (DOC-02/10/12/16), Kurulum/Operasyon/Release (DOC-03/04/06/13/14/15), Güvenlik/Devir/Yönetişim (DOC-05/07/08/09/11). Her alanın belgeleri, bir başka alan tarafından bağımsız olarak yeniden incelendi: Mimari/Veri/Modüller belgeleri Kurulum/Operasyon/Release açısından incelendi; Kurulum/Operasyon/Release belgeleri Güvenlik/Devir/Yönetişim açısından incelendi; Güvenlik/Devir/Yönetişim belgeleri Mimari/Veri/Modüller açısından incelendi. İnceleme yalnızca metin okumakla kalmadı, iddialar doğrudan kod/script üzerinden yeniden doğrulandı. Aşağıdaki **14 bulgu**, tümü **çözüldü** olarak işaretlenmiştir.
...
| 14 | HANDOVER_GAP | DOC-03 §4, DOC-15 §7 | Redis'in opsiyonel olduğu sorusu ayrıca doğrulanmak üzere ertelenmişti; artık DOC-02 §6'da CODE_VERIFIED olarak yanıtlanmış | Her iki belge güncellendi, doğrudan CODE_VERIFIED sonuca ve DOC-02 §6'ya atıf |

**CROSS_REVIEW_FINDING_COUNT = 14, CROSS_REVIEW_RESOLVED_COUNT = 14.**
...
| PT72H sertleştirmesi | DOC-03, DOC-13, DOC-15, DOC-19 — dördü de tutarlı biçimde raporluyor: canlı Scheduled Task'ta **uygulanmış ve mekanik doğrulanmış** (`ExecutionTimeLimit=PT0S`, `RestartCount=3`, `RestartInterval=PT1M`); yalnızca repo kurulum script'inin bu parametreleri henüz geçirmemesi ayrı, düşük öncelikli bir açık madde olarak kalıyor. (DOC-14, konuyu hiç ele almıyor — ilgisiz, çelişki değil.) | UYUMLU |
| Redis opsiyonel | DOC-02 (kaynak, CODE_VERIFIED); DOC-03/DOC-07/DOC-08/DOC-15 çapraz inceleme sırasında CODE_VERIFIED'e güncellendi (bulgu #14, #5) | UYUMLU |
...
**CROSS_DOCUMENT_CONSISTENCY_RESULT: PASS.** 1 çelişki önceki fazda, 14 bulgu bir sonraki fazda (çapraz inceleme), 1 kurumsal olgu düzeltmesi (KYS) + 1 numaralandırma kusuru (DOC-01) bu fazda tespit edilmiş; tümü (17/17) kaynağında düzeltilmiş ve bu matriste kayıt altına alınmıştır. KYS düzeltmesi, kullanıcının doğrudan yetkili beyanına dayanır (kod/repo içinden bağımsız doğrulanabilir bir olgu değildir); diğer tüm düzeltmeler bu incelemede mekanik olarak (kod/script okuma veya başlık numarası tarama) doğrulanmıştır.
