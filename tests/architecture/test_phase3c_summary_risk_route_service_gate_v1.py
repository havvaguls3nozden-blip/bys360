...
from scripts.quality.bys360_phase3c_summary_risk_route_service_gate_v1 import run_checks


def test_phase3c_summary_risk_route_service_gate_v1() -> None:
    root = Path(__file__).resolve().parents[2]
    result = run_checks(root, write_report=False)

    assert result["summary_risk_route_service_gate_ok"] is True
    assert result["route_decorator_count"] == 22
    assert result["route_import_ok"] is True
    assert result["deps_function_ok"] is True
    assert result["route_globals_bridge_ok"] is True
