...
def _bys360_method_aware_route_keys(path):
    ...
    pattern = re.compile(
        r'@(?:mobile_api_bp|mobile_bp)\.(get|post|put|patch|delete|route)\(\s*[\'"]([^\'"]+)[\'"](?P<rest>[^)]*)\)',
    ...
    for match in pattern.finditer(source):
        ...
        if decorator == "route":
            methods = []
            method_match = re.search(r"methods\s*=\s*(\[[^\]]+\]|\([^)]+\))", rest, re.DOTALL)
            if method_match:
                try:
                    parsed = ast.literal_eval(method_match.group(1))
                    methods = [str(item).upper() for item in parsed]
                except Exception:
                    methods = []
            if not methods:
                methods = ["GET"]
...
def test_mobile_route_contract_count():
    ...
    for path in _all_mobile_route_files():
        ...
        route_rules.extend(_bys360_method_aware_route_keys(path))
