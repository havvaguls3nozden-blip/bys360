...
(function(){
  'use strict';
  var VERSION = 'BYS360_AI_EVERYWHERE_V1';
  ...
  function openAssistant(question){
    try{
      var launcher = document.querySelector('#bys360-assistant-module-root .bys360-am-launcher, .bys360-am-launcher, [data-bys360-ai-agent-launcher], .bys360-ai-agent-launcher');
      if(launcher){ launcher.click(); }
      window.setTimeout(function(){
        var root = document.querySelector('#bys360-assistant-module-root, [data-bys360-assistant-module]');
        var input = root ? root.querySelector('[data-chat-input], textarea, input[type="text"]') : document.querySelector('[data-chat-input]');
        var form = root ? root.querySelector('[data-chat-form]') : null;
        if(input){
          input.value = question || 'Bu ekranda ne yapabilirim?';
  ...
  function render(){
    ...
    card.innerHTML = ''+
      '<div class="bys360-ai-everywhere-inner">'+
        ...
        '</div>'+ renderLinks(rule.links || DEFAULT_RULE.links) +
      '</div>';
  ...
  document.addEventListener('click', function(ev){
    ...
    if(ask){
      ev.preventDefault();
      openAssistant(ask.getAttribute('data-ai-everywhere-ask') || 'Bu ekranda ne yapabilirim?');
      return;
    }
  ...
  window.BYS360AIEverywhereV1 = {version:VERSION, getRule:getRule, openAssistant:openAssistant, render:render};
})();
