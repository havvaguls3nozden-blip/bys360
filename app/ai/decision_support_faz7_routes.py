...
def _run_faz7_json(builder: ResponseBuilder, *args: Any) -> tuple[Any, int]:
    try:
        ...
        logger.exception("BYS360 V6C guarded exception | file=app/ai/decision_support_faz7_routes.py | line=43")
        safe_db_rollback()
        return jsonify({"ok": False, "error": f"Geçmiş karne karar destek kontrolünde beklenmeyen hata: {exc}"}), 500
