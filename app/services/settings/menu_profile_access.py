...
def save_user_menu_overrides_handler(
    *,
    ...
    """Kisi bazli menu override kayitlarini yalniz fark olan satirlarla saklar."""
    # BYS360_PERSONNEL_FEATURE_MATRIX_V1_3_DEDUP_SERVICE
    # Kişi bazlı rol matrisi kaydedilirken aynı menu_key birden fazla satırdan gelirse
    # önce tekilleştirir. Böylece (user_id, menu_key) unique constraint hatası oluşmaz.
    _seen_menu_keys = set()
    _deduped_flat_menu_items = []
    for _item in flat_menu_items or []:
        _menu_key = str((_item or {}).get("key") or "").strip()
        if not _menu_key or is_removed_menu_key_func(_menu_key) or _menu_key in _seen_menu_keys:
            continue
    ...
    base_context = build_base_rule_map_for_user_func(user, flat_menu_items)
    base_rule_map = base_context["base_rule_map"]
    existing = {
        row.menu_key: row
        for row in filter_live_menu_rows_func(user_menu_permission_model.query.filter_by(user_id=user.id).all())
    }
    changed = 0
    ...
    for item in flat_menu_items:
        menu_key = item["key"]
        desired = menu_key in visible_keys
        base_visible = bool(base_rule_map.get(menu_key, False))
        row = existing.get(menu_key)
        if desired == base_visible:
            if row is not None:
                db_session.delete(row)
                deleted_keys.add(menu_key)
                changed += 1
            continue
        keep_keys.add(menu_key)
        if row is None:
            ...
            changed += 1
        elif row.is_visible != desired or (row.source_type or "") != "user_override":
            row.is_visible = desired
            row.source_type = "user_override"
            changed += 1
    for menu_key, row in existing.items():
        if menu_key in deleted_keys:
            continue
        if menu_key not in keep_keys and row is not None:
            db_session.delete(row)
            changed += 1
    new_state = {menu_key: (menu_key in visible_keys) for menu_key in keep_keys}
    create_settings_change_log_func(
        actor_user_id=updated_by_user_id,
        change_scope="user_menu_overrides",
        action_type="save",
        summary=f"Kullanıcı override güncellendi: {getattr(user, 'ad', '')} {getattr(user, 'soyad', '')}".strip(),
        previous_state=previous_state,
        new_state=new_state,
        target_user_id=user.id,
    )
    db_session.commit()
    return {"changed": changed, "override_count": len(keep_keys)}
