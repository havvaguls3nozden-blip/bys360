...
def tone(avg: float | None) -> str:
    if avg is None: return 'neutral'
    if avg >= 4.25: return 'good'
    if avg >= 3.45: return 'stable'
    if avg >= 2.75: return 'watch'
    return 'risk'
...
def build_pulse_analytics_decision_support(analytics: dict[str, Any] | None, days: int = 30) -> dict[str, Any]:
    a = analytics or {}; avg = _f(_get(a,'average')); delta = _f(_get(a,'delta'), 0); cov = _f(_get(a,'coverage_rate')); risk = _i(_get(a,'risk_user_count')); eligible = bool(_get(a,'eligible'))
    t = tone(avg)
    if not eligible:
    ...
    flags = []
    if cov is not None and cov < 40: flags.append('Kapsama oranı düşük; temkinli yorumlanmalı.')
    if delta is not None and delta < -0.5: flags.append('Son 7 gün eğilimi belirgin düşüş gösteriyor.')
    if risk: flags.append(f'{risk} risk serisi izleniyor.')
    if not flags: flags.append('Kritik otomatik uyarı yok.')
...
def build_results_decision_support(campaign: Any, results: list[dict[str, Any]] | None, pulse_context: dict[str, Any] | None) -> dict[str, Any]:
    ...
    low, high = [], []
    for r in rows:
        avg = _f(r.get('average')); q = str(_get(r.get('question'),'question_text','Soru'))
        if avg is not None and avg <= 2.7: low.append(q)
        if avg is not None and avg >= 4.2: high.append(q)
    ...
    if low:
        summary='Düşük bantta soru var. Kök neden ve aksiyon planı önerilir.'; actions=['Düşük ortalamalı soruları aksiyona dönüştürün.','Metin yanıtlarında tekrar eden konuları çıkarın.','Sonraki kampanyada takip sorusu açın.']
    else:
        summary='Sonuçlar yönetilebilir görünüyor. Güçlü alanlar iyi uygulama olarak işaretlenebilir.'; actions=['Güçlü alanları iyi uygulama olarak kaydedin.','Katılım düşükse hatırlatma yapın.','Metinleri olumlu/iyileştirme başlığına ayırın.']
    flags=[]
    if total < 5: flags.append('Yanıt sayısı düşük; genelleme sınırlı.')
    if low: flags.append(f'Düşük bantta {len(low)} soru var.')
    if high: flags.append(f'Güçlü bantta {len(high)} soru var.')
    if not flags: flags.append('Kritik sonuç uyarısı yok.')
...
def build_manager_decision_support(summary: dict[str, Any] | None) -> dict[str, Any]:
    s = summary or {}; base = build_pulse_analytics_decision_support(s.get('analytics') or {}, 30)
    ...
    if zero: actions.insert(0, 'Yanıt almayan kampanyaların hedef kitle ve duyuru görünürlüğünü kontrol edin.')
    flags = list(base.get('flags') or [])
    if zero: flags.append(f'Yanıt almayan kampanya sayısı: {len(zero)}')
