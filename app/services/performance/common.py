...
def normalize_weight_inputs(
    ...
    w3 = max(0.0, _safe_float(evaluator_3_weight, 0.0))
    ...
    if not level_3_enabled:
        ...
        w3 = 0.0
        level_3_mode = LEVEL_3_DEFAULT_MODE

    total = w1 + w2 + w3
    if total <= 0:
        w1, w2, w3 = DEFAULT_THREE_MANAGER_SCORING_WEIGHTS if level_3_mode == "scoring" else DEFAULT_TWO_MANAGER_WEIGHTS
        total = 100.0
    ...
    total_after = normalized["evaluator_1_weight"] + normalized["evaluator_2_weight"] + normalized["evaluator_3_weight"]
    if total_after != 100.0:
        normalized["evaluator_1_weight"] = round(normalized["evaluator_1_weight"] + (100.0 - total_after), 2)
    return normalized


def get_period_level_3_flags(
    period: Optional[PerformancePeriod] = None,
    weight_config: Optional[PerformanceWeightConfig] = None,
) -> Dict[str, Any]:
    ...
    # BYS360_PHASE4_THIRD_SUPERVISOR_POLICY_GATE
    try:
        ...
        enabled = bool(_phase4_policy.get("enabled"))
        mode = str(_phase4_policy.get("mode") or "off")
        scoring_enabled = bool(_phase4_policy.get("scoring_enabled"))
    except Exception:
        logger.exception("BYS360 performans modülünde beklenmeyen hata yakalandı.")
        import logging
...
def calculate_effective_weights(
    period: Optional[PerformancePeriod],
    ...
    if manager_3_id and flags["enabled"] and flags["scoring_enabled"]:
        active_levels[3] = base["evaluator_3_weight"]
    ...
    if total <= 0:
        if 1 in active_levels and 2 in active_levels:
            active_levels = {1: 50.0, 2: 50.0}
        elif 1 in active_levels:
            active_levels = {1: 100.0}
        elif 2 in active_levels:
            active_levels = {2: 100.0}
        else:
            active_levels = {3: 100.0}
        total = sum(active_levels.values())
    ...
    total_after = sum(effective.values())
    if total_after != 100.0 and any(effective.values()):
        effective["evaluator_1_weight"] = round(effective["evaluator_1_weight"] + (100.0 - total_after), 2)
    return effective
