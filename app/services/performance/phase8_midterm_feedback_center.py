...
def phase8_schema_sql() -> str:
    return f"""
CREATE TABLE IF NOT EXISTS {PHASE8_TABLE_NAME} (
    ...
    visibility VARCHAR(60) NOT NULL DEFAULT 'private_manager',
    status VARCHAR(40) NOT NULL DEFAULT 'active',
    remind_evaluator BOOLEAN NOT NULL DEFAULT TRUE,
...
def ensure_phase8_schema() -> dict[str, Any]:
    try:
        from sqlalchemy import text

        from app import db
    except Exception as exc:
        logger.exception("BYS360 V6C guarded exception | file=app/services/performance/phase8_midterm_feedback_center.py | line=419")
        return {"ok": False, "error": str(exc), "table": PHASE8_TABLE_NAME}
    try:
        db.session.execute(text(phase8_schema_sql()))
        db.session.commit()
        return {"ok": True, "table": PHASE8_TABLE_NAME}
    except Exception as exc:
        ...
        try:
            ...
            logger.exception("BYS360 performans modülünde beklenmeyen hata yakalandı.")
            logging.getLogger(__name__).exception("BYS360 suppressed exception captured in app/services/performance/phase8_midterm_feedback_center.py:427")
        return {"ok": False, "error": str(exc), "table": PHASE8_TABLE_NAME}


def seed_phase8_midterm_feedback_settings() -> dict[str, Any]:
    changed: list[str] = []
    ...
    schema = ensure_phase8_schema()
    for module_key, setting_key, label, value_type, default_value, description in PHASE8_SETTING_ROWS:
        try:
            ...
            logger.exception("BYS360 V6C guarded exception | file=app/services/performance/phase8_midterm_feedback_center.py | line=464")
            continue
    try:
        db.session.commit()
        ...
        try:
            ...
            logger.exception("BYS360 performans modülünde beklenmeyen hata yakalandı.")
            logging.getLogger(__name__).exception("BYS360 suppressed exception captured in app/services/performance/phase8_midterm_feedback_center.py:471")
        return {"ok": False, "error": str(exc), "settings": changed, "schema": schema}
