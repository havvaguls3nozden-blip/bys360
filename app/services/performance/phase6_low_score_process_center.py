...
def classify_phase6_repeat(previous_low_count_in_year: Any = 0, sequence_no: Any = None) -> tuple[str,str]:
    if _safe_int(sequence_no,0) >= 2: return "second_low_repeat", STATUS_LABELS["second_low_repeat"]
...
def resolve_phase6_low_score_process(*, final_score: Any, approval_status: Any=None, process_record_exists: bool=False, warning_record_exists: bool=False, administrative_process_exists: bool=False, previous_low_count_in_year: Any=0, sequence_no: Any=None) -> Phase6Decision:
    ...
    warning_key,warning_label=classify_phase6_repeat(previous_low_count_in_year,sequence_no); is_second=warning_key=="second_low_repeat"
    if rejected:
...
def resolve_phase6_process_object(process: Any) -> Phase6Decision:
    final_score=getattr(process,"final_score",None); status="president_pending"
    if process_is_rejected(process): status="rejected_by_president"
    elif process_is_approved(process): status="approved_by_president"
    warning_exists=process_has_first_warning_record(process); admin_exists=process_has_second_admin_record(process)
...
def phase6_process_steps(decision: Phase6Decision) -> list[dict[str,Any]]:
    ...
    for idx,(key,title) in enumerate(PROCESS_STEPS,start=1):
        ...
        elif key=="publish_ready" and decision.can_publish: state="done"
...
def seed_phase6_low_score_process_settings() -> dict[str,Any]:
    ...
    for module_key,setting_key,label,value_type,default_value,description in PHASE6_SETTING_ROWS:
        try:
            ...
            if row is None:
                row=ModuleSetting(module_key=module_key,setting_key=setting_key); db.session.add(row)
            if hasattr(row,"label"): row.label=label
            if hasattr(row,"value_type"): row.value_type=value_type
            if hasattr(row,"value_text") and not getattr(row,"value_text",None): row.value_text=default_value
            if hasattr(row,"description"): row.description=description
            if hasattr(row,"is_active"): row.is_active=True
            changed.append(f"{module_key}.{setting_key}")
        except Exception: continue
    try:
        db.session.commit(); return {"ok":True,"settings":changed}
