...
def executive_mail_center_home():
    _require()
    ensure_defaults(getattr(current_user, "id", None))
...
def executive_mail_center_location():
    _require()
    save_location_from_form(request.form, getattr(current_user, "id", None))
...
def executive_mail_center_send_task(task_key):
    _require()
    result = run_task(task_key, dry_run=False, actor_user_id=getattr(current_user, "id", None))
...
def executive_mail_center_logs():
    _require()
    return render_template("executive_summary/mail_center/logs.html", **dashboard_context(request.args.get("q")))

@main_bp.route("/executive-summary/mail-center/scheduler")
@login_required
def executive_mail_center_scheduler():
    _require()
    return render_template("executive_summary/mail_center/scheduler.html", **dashboard_context(request.args.get("q")))
