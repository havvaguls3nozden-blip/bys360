...
$releaseCode = "BYS360_CIC_V3_0_LIVE_FINAL_RELEASE_RESUME_V3"
...
# Bu paket servis dosyasını doğrudan taşır. Canlıda Faz 3 service dosyası eksikse burada tamamlanır.
$servicePath = Join-Path $ProjectRoot "app\services\corporate_information_center.py"
if (-not (Test-Path $servicePath)) {
    throw "Bootstrap servis dosyası kopyalanamadı: $servicePath"
}
Write-Host "Bootstrap servis dosyasi hazir: $servicePath"
...
# Sıra özellikle düzeltildi: önce şablon/servis omurgası, sonra CSRF form token.
$steps = @(
...
foreach ($step in $steps) {
    ...
    if ($exit -ne 0) {
        if ($step.AllowFail) {
            Write-Warning "Beklenen/telafi edilebilir gate uyarisi alindi, sonraki duzeltme paketiyle devam ediliyor: $($step.Zip) ExitCode=$exit"
        } else {
...
$finalCheck = Join-Path $ProjectRoot "scripts\quality\check_bys360_cic_v3_0_live_final_release_resume_v3.py"
if (Test-Path $finalCheck) {
    $py = Join-Path $ProjectRoot ".venv\Scripts\python.exe"
    if (-not (Test-Path $py)) { $py = "python" }
    & $py $finalCheck -ProjectRoot $ProjectRoot
    if ($LASTEXITCODE -ne 0) { throw "Resume V3 final kontrolu basarisiz." }
}

$py2 = Join-Path $ProjectRoot ".venv\Scripts\python.exe"
if (-not (Test-Path $py2)) { $py2 = "python" }
& $py2 -m compileall app scripts | Out-Host
if ($LASTEXITCODE -ne 0) { throw "compileall basarisiz." }

Write-Host "BYS360_CIC_V3_0_LIVE_FINAL_RELEASE_RESUME_V3_GATE_OK"
Write-Host "BYS360_CIC_V3_0_LIVE_FINAL_RELEASE_RESUME_V3_FINAL_OK"
