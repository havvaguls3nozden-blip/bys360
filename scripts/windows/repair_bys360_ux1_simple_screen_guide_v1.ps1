...
Write-Host "BYS360 Phase UX-1 - Sade Ekran ve Kullanici Mantigi Temizligi"
...
<link rel="stylesheet" href="{{ url_for('static', filename='css/bys360_ux1_simple_screen_guide.css') }}?v=ux1-simple-screen-guide-v1">
<!-- /BYS360_UX1_SIMPLE_SCREEN_GUIDE_CSS -->
'@
    $jsBlock = @'
<!-- BYS360_UX1_SIMPLE_SCREEN_GUIDE_JS -->
<script src="{{ url_for('static', filename='js/bys360_ux1_simple_screen_guide.js') }}?v=ux1-simple-screen-guide-v1" defer></script>
<!-- /BYS360_UX1_SIMPLE_SCREEN_GUIDE_JS -->
'@
...
function Verify-Fix {
    ...
    $checks.js_has_contexts = $js.Contains("Performans işlemleri") -and $js.Contains("Personel ve organizasyon işlemleri")
    $checks.js_has_long_text_simplifier = $js.Contains("simplifyLongTextBlocks") -and $js.Contains("Detayı göster")
    ...
    $result = [ordered]@{
        package = "BYS360_UX1_SIMPLE_SCREEN_GUIDE_V1"
        generated_at = (Get-Date).ToString("s")
        ok = $ok
        checks = $checks
        rule = "Kullanici ilk bakista ekranin amacini anlamali; uzun aciklamalar varsayilan olarak kisaltilmali, detay kullanici isterse acilmalidir."
    }
    $result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $ReportPath -Encoding UTF8
    $result | ConvertTo-Json -Depth 6
    if (-not $ok) { throw "UX-1 sade ekran kontrat kontrolu basarisiz." }
}
...
Write-Host ""
Write-Host "UX-1 TAMAM: Ekranlara kisa mantik karti, adimlar ve uzun metinleri ac/kapat standardi eklendi."
