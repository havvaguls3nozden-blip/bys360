...
$releaseCode = "BYS360_CIC_V3_0_LIVE_FINAL_RELEASE_RESUME_V2"
Write-Host "$releaseCode uygulanıyor..."
Write-Host "ProjectRoot=$ProjectRoot"

if (-not (Test-Path $ProjectRoot)) { throw "ProjectRoot bulunamadi: $ProjectRoot" }

# Overlay zip konumu: eski paket bazen overlays, bazen scripts\overlays altına açılmış olabilir.
...
# ÖNEMLİ: CSRF form token paketi şablonlar oluştuktan sonra çalışmalı.
# Eski final scriptte V2.15.15 erken çalıştığı için canlıda eksik şablon hatası alındı.
$steps = @(
...
$finalCheck = Join-Path $ProjectRoot "scripts\quality\check_bys360_cic_v3_0_live_final_release_resume_v2.py"
if (Test-Path $finalCheck) {
    & (Join-Path $ProjectRoot ".venv\Scripts\python.exe") $finalCheck -ProjectRoot $ProjectRoot
    if ($LASTEXITCODE -ne 0) { throw "Resume V2 final kontrolu basarisiz." }
}
...
Write-Host "BYS360_CIC_V3_0_LIVE_FINAL_RELEASE_RESUME_V2_GATE_OK"
Write-Host "BYS360_CIC_V3_0_LIVE_FINAL_RELEASE_RESUME_V2_FINAL_OK"
