...
IMPORT_SENTINEL = "# BYS360 P11-B9: performance read mobile routes bridge"
CALL_LINE = "_register_mobile_performance_read_routes_v1(mobile_bp, globals())"
...
def make_new_module_text(function_sources: list[str]) -> str:
    ...
    parts = [
        ...
        "# P11-B9 kapsamında küçük GET performans/rapor okuma endpointleri ayrılmıştır.",
        ...
        "",
        "",
        "def register_mobile_performance_read_routes_v1(mobile_bp, route_globals: dict) -> None:",
        "    \"\"\"Register performance read mobile routes on the existing mobile blueprint.\"\"\"",
        "    if route_globals.get(\"_BYS360_P11_B9_PERFORMANCE_READ_ROUTES_REGISTERED\"):",
        "        return",
        "",
        "    route_globals[\"mobile_bp\"] = mobile_bp",
        "    exec(_PERFORMANCE_READ_ROUTE_SOURCE, route_globals, route_globals)",
        "    route_globals[\"_BYS360_P11_B9_PERFORMANCE_READ_ROUTES_REGISTERED\"] = True",
        "",
    ]
    return "\n".join(parts)
...
def apply(project_root: Path, dry_run: bool) -> None:
    ...
    if IMPORT_SENTINEL in text and new_module.exists():
        print("BYS360_QUALITY_10_10_P11_B9_ALREADY_APPLIED")
        return
    ...
    for start, end, name in ranges:
        ...
        output_lines.append(f"# BYS360 P11-B9: {name} performance read route app/api/mobile/performance_read_routes.py modülüne taşındı.")
        current = end + 1
    ...
    print("BYS360_QUALITY_10_10_P11_B9_MOBILE_PERFORMANCE_READ_ROUTES_BRIDGE_START")
    ...
    if dry_run:
        print("BYS360_QUALITY_10_10_P11_B9_DRYRUN_OK")
        return

    stamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_root = project_root / ".quality_backup" / f"p11_b9_mobile_performance_read_routes_bridge_{stamp}"
    ...
    report_json = out_dir / "bys360_quality_10_10_p11_b9_mobile_performance_read_routes_bridge_v1.json"
    report_md = out_dir / "bys360_quality_10_10_p11_b9_mobile_performance_read_routes_bridge_v1.md"
    write_text(report_json, json.dumps(report, ensure_ascii=False, indent=2))

    md = []
    md.append("# BYS360 P11-B9 Mobile Performance Read Routes Bridge")
    md.append("")
    ...
    md.append("## Güvenlik")
    md.append("")
    md.append("- Sadece küçük GET performans/rapor okuma endpointleri taşınmıştır.")
    md.append("- Personel listesi, auth, token, password, commit/rollback ve POST endpointlere dokunulmamıştır.")
    ...
    print("BYS360_QUALITY_10_10_P11_B9_MOBILE_PERFORMANCE_READ_ROUTES_BRIDGE_OK")
