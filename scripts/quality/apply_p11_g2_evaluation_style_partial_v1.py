...
PARTIAL_REL = "app/templates/partials/evaluation_form/_evaluation_form_styles.html"
INCLUDE_LINE = "{% include 'partials/evaluation_form/_evaluation_form_styles.html' %}"
...
def main() -> int:
    ...
    print("BYS360_QUALITY_10_10_P11_G2_EVALUATION_STYLE_PARTIAL_START")
    ...
    if INCLUDE_LINE in text and not re.search(r"(?is)<style\b[^>]*>.*?</style>", text):
        print("BYS360_QUALITY_10_10_P11_G2_ALREADY_APPLIED")
        return 0
    ...
    # Basic safety guards.
    if style_line_count < 50:
        raise SystemExit("STYLE_BLOCK_TOO_SMALL_FOR_P11_G2")
    if "<script" in style_block.lower():
        raise SystemExit("STYLE_BLOCK_CONTAINS_SCRIPT_UNSAFE")
    if "<form" in style_block.lower() or "name=" in style_block.lower() or "csrf" in style_block.lower():
        raise SystemExit("STYLE_BLOCK_CONTAINS_FORM_HINT_UNSAFE")
    if expected_reduction < 100:
        raise SystemExit("EXPECTED_REDUCTION_TOO_SMALL")

    if args.dry_run:
        print("BYS360_QUALITY_10_10_P11_G2_DRYRUN_OK")
        return 0

    stamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_root = project_root / ".quality_backup" / f"p11_g2_evaluation_style_partial_{stamp}"
    ...
    header = (
        "{# BYS360 P11-G2: evaluation_form inline style bloğu güvenli partial olarak ayrıldı. #}\n"
        "{# Form, input, name/id, CSRF ve script davranışları bu dosyaya taşınmamıştır. #}\n"
    )
    ...
    report_json = out_dir / "bys360_quality_10_10_p11_g2_evaluation_style_partial_v1.json"
    report_md = out_dir / "bys360_quality_10_10_p11_g2_evaluation_style_partial_v1.md"
    report_json.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    md = []
    md.append("# BYS360 P11-G2 Evaluation Form Style Partial")
    md.append("")
    ...
    md.append("")
    md.append("Bu işlem yalnızca inline `<style>` bloğunu partial dosyaya taşır. Form, input, name/id, CSRF, Jinja puanlama döngüsü ve script davranışlarına dokunmaz.")
    report_md.write_text("\n".join(md) + "\n", encoding="utf-8")

    print(f"backup_root={backup_root}")
    print(f"style_partial_report_json={report_json}")
    print(f"style_partial_report_md={report_md}")
    print("BYS360_QUALITY_10_10_P11_G2_EVALUATION_STYLE_PARTIAL_OK")
    return 0
