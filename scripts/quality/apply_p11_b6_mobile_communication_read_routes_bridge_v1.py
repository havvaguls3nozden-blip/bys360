...
NEW_MODULE_REL = "app/api/mobile/communication_read_routes.py"

TARGET_FUNCTIONS = [
    "mobile_notifications",
    "mobile_b46_communication_message_threads",
    "mobile_b46_communication_users",
]

IMPORT_SENTINEL = "# BYS360 P11-B6: communication read mobile routes bridge"
CALL_LINE = "_register_mobile_communication_read_routes_v1(mobile_bp, globals())"
...
def make_new_module_text(function_sources: list[str]) -> str:
    ...
    parts = [
        ...
        "from __future__ import annotations",
        "",
        "_COMMUNICATION_READ_ROUTE_SOURCE = " + repr(source_blob),
        "",
        "",
        "def register_mobile_communication_read_routes_v1(mobile_bp, route_globals: dict) -> None:",
        ...
        "    exec(_COMMUNICATION_READ_ROUTE_SOURCE, route_globals, route_globals)",
...
def apply(project_root: Path, dry_run: bool) -> None:
    ...
    if IMPORT_SENTINEL in text and new_module.exists():
        print("BYS360_QUALITY_10_10_P11_B6_ALREADY_APPLIED")
        return
    ...
    for start, end, name in ranges:
        ...
        output_lines.append(f"# BYS360 P11-B6: {name} communication read route app/api/mobile/communication_read_routes.py modülüne taşındı.")
        current = end + 1
    ...
    print("BYS360_QUALITY_10_10_P11_B6_MOBILE_COMMUNICATION_READ_ROUTES_BRIDGE_START")
    ...
    if dry_run:
        print("BYS360_QUALITY_10_10_P11_B6_DRYRUN_OK")
        return

    stamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_root = project_root / ".quality_backup" / f"p11_b6_mobile_communication_read_routes_bridge_{stamp}"
    ...
    report_json = out_dir / "bys360_quality_10_10_p11_b6_mobile_communication_read_routes_bridge_v1.json"
    report_md = out_dir / "bys360_quality_10_10_p11_b6_mobile_communication_read_routes_bridge_v1.md"
    write_text(report_json, json.dumps(report, ensure_ascii=False, indent=2))

    md = []
    md.append("# BYS360 P11-B6 Mobile Communication Read Routes Bridge")
    md.append("")
    ...
    md.append("## Güvenlik")
    md.append("")
    md.append("- Sadece GET iletişim/bildirim okuma endpointleri taşınmıştır.")
    ...
    print(f"communication_read_routes_bridge_report_json={report_json}")
    print(f"communication_read_routes_bridge_report_md={report_md}")
    print("BYS360_QUALITY_10_10_P11_B6_MOBILE_COMMUNICATION_READ_ROUTES_BRIDGE_OK")
