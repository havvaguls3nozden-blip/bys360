...
P4A auth-guard matrix ve P4B role-boundary matrix raporlarını tek CI standardında
birleştirir. Canlı veriye yazmaz; yalnızca mevcut raporlar, mobil route sözleşmesi,
compile/app factory/secret/pytest kapılarını doğrular.
"""
...
PACKAGE = "BYS360_CLAUDE_SCORE_UPLIFT_P4C_MOBILE_SECURITY_SUITE_GATE"
...
def _read_json(path: Path) -> Dict[str, Any]:
    ...
    except Exception as exc:  # pragma: no cover - defensive CI report parsing
...
def _extract_probe_summary(report: Dict[str, Any], matrix_key: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "probe_count": matrix.get("probe_count", len(responses)),
...
def _security_suite(root: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "total_probe_count": int(p4a_summary.get("probe_count") or 0) + int(p4b_summary.get("probe_count") or 0),
        "feature_coverage": {
...
def _pytest_gate(root: Path, python: str) -> Dict[str, Any]:
    ...
    result["mode"] = "pytest_targeted_mobile_security_suite_p4c"
    return result
...
def build_report(root: Path, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    report: Dict[str, Any] = {
        ...
        "ci_commands": [
            ...
            "python scripts/quality/bys360_mobile_security_suite_gate_p4c.py --root . --compile-all --app-factory --secret-gate --pytest-gate",
        ],
        "report": str((root / REPORT_REL).resolve()),
        "next_actions": [
            "P4C temizse P4A ve P4B V3 mobil guvenlik kapilari tek suite runner altinda standart kabul edilebilir.",
