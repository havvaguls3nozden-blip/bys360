...
NEW_MODULE_REL = "app/api/mobile/utility_routes.py"
TARGET_FUNCTIONS = ["mobile_health", "mobile_kpi_goals"]

IMPORT_SENTINEL = "# BYS360 P11-B2: utility mobile routes bridge"
CALL_LINE = "_register_mobile_utility_routes_v1(mobile_bp, globals())"
...
def route_decorator_names(node: ast.AST) -> list[str]:
...
def validate_target(node: ast.AST, source: str) -> None:
    ...
    names = route_decorator_names(node)
    ...
    risky = ["commit(", "rollback(", "set_password", "password", "db.session.add", "delete("]
...
def make_new_module_text(function_sources: list[str]) -> str:
    ...
    parts = [
        ...
        "from __future__ import annotations",
        "",
        "_UTILITY_ROUTE_SOURCE = " + repr(source_blob),
        "",
        "",
...
def apply(project_root: Path, dry_run: bool) -> None:
    ...
    if IMPORT_SENTINEL in text and new_module.exists():
        print("BYS360_QUALITY_10_10_P11_B2_ALREADY_APPLIED")
        return
    ...
    for start, end, name in ranges:
        ...
        output_lines.append(f"# BYS360 P11-B2: {name} utility route app/api/mobile/utility_routes.py modülüne taşındı.")
        current = end + 1
    ...
    print("BYS360_QUALITY_10_10_P11_B2_MOBILE_UTILITY_ROUTES_BRIDGE_START")
    ...
    if dry_run:
        print("BYS360_QUALITY_10_10_P11_B2_DRYRUN_OK")
        return

    stamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_root = project_root / ".quality_backup" / f"p11_b2_mobile_utility_routes_bridge_{stamp}"
    ...
    report_json = out_dir / "bys360_quality_10_10_p11_b2_mobile_utility_routes_bridge_v1.json"
    report_md = out_dir / "bys360_quality_10_10_p11_b2_mobile_utility_routes_bridge_v1.md"
    write_text(report_json, json.dumps(report, ensure_ascii=False, indent=2))

    md = []
    md.append("# BYS360 P11-B2 Mobile Utility Routes Bridge")
    md.append("")
    ...
    md.append("## Güvenlik")
    md.append("")
    md.append("- Sadece düşük riskli GET utility endpointleri taşınmıştır.")
    md.append("- Auth, personel oluşturma, password, commit/rollback ve POST endpointlere dokunulmamıştır.")
    ...
    print(f"utility_routes_bridge_report_json={report_json}")
    print(f"utility_routes_bridge_report_md={report_md}")
    print("BYS360_QUALITY_10_10_P11_B2_MOBILE_UTILITY_ROUTES_BRIDGE_OK")
