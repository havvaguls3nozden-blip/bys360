...
PACKAGE = "BYS360_PHASE3B_MOBILE_PERFORMANCE_ITEM_HELPERS_GATE_V1"
REPORT_REL = Path("reports/architecture/BYS360_PHASE3B_MOBILE_PERFORMANCE_ITEM_HELPERS_GATE_V1_REPORT.json")

ROOT_REL_ROUTE = Path("app/api/mobile/performance_routes.py")
ROOT_REL_SERVICE = Path("app/api/mobile/services/performance_item_helpers.py")
...
def run_checks(root: Path, write_report: bool = True) -> dict[str, Any]:
    ...
    route_import_ok = "from app.api.mobile.services.performance_item_helpers import" in route_text
    ...
    result: dict[str, Any] = {
        ...
        "route_line_reduction_ok": len(route_text.splitlines()) < 1637,
    ...
    result["item_helpers_gate_ok"] = bool(
        result["route_exists"]
...
def main() -> int:
    ...
    return 0 if result["item_helpers_gate_ok"] else 1
