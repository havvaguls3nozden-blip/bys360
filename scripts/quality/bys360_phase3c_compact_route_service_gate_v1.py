...
PACKAGE = "BYS360_PHASE3C_COMPACT_ROUTE_SERVICE_GATE_V1"
REPORT_REL = Path("reports/architecture/BYS360_PHASE3C_COMPACT_ROUTE_SERVICE_GATE_V1_REPORT.json")

ROUTE_REL = Path("app/api/mobile/performance_routes.py")
SERVICE_REL = Path("app/api/mobile/services/performance_compact_route_services.py")

TARGET_ROUTE_FUNCTIONS = [
    "mobile_performance_approvals",
    "mobile_performance_criteria",
    "_bys360_legacy_mobile_performance_in_period_notes",
]
...
def run_checks(root: Path, write_report: bool = True) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "route_import_ok": "performance_compact_route_services" in route_text,
        "globals_bridge_ok": "globals())" in route_text and "deps[" in service_text,
        ...
        "service_logic_ok": all(token in service_text for token in [
            "PerformancePresidentApproval.query",
        ...
        "route_line_reduction_ok": len(route_text.splitlines()) < 1170,
        "compact_route_service_gate_ok": False,
    }

    result["compact_route_service_gate_ok"] = bool(
        result["route_exists"]
        and result["service_exists"]
        and result["route_decorator_count"] == 22
        and result["route_import_ok"]
        and result["globals_bridge_ok"]
...
def main() -> int:
    ...
    return 0 if result["compact_route_service_gate_ok"] else 1
