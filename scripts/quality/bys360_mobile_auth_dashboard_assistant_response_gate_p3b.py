...
PACKAGE = "BYS360_CLAUDE_SCORE_UPLIFT_P3B_MOBILE_AUTH_DASHBOARD_ASSISTANT_RESPONSE_GATE"
...
'''
    ...
    for method, full_path, _domain in EXPECTED_ROUTES:
        found = [r for r in routes if r.get("rule") == full_path]
        if not found:
            missing.append(f"{method} {full_path}")
        elif method not in found[0].get("methods", []):
            method_mismatches.append(f"{method} {full_path}")
    return {**result, "runtime_route_count": len(routes), "missing_runtime_routes": missing, "runtime_method_mismatches": method_mismatches, "ok": result["ok"] and not missing and not method_mismatches}
...
def run_pytest(root: Path) -> dict[str, Any]:
    test_path = "tests/architecture/test_mobile_auth_dashboard_assistant_response_p3b.py"
    result = _run_subprocess([sys.executable, "-m", "pytest", test_path, "-q"], root, env=_smoke_env(root), timeout=120)
    result["mode"] = "pytest_targeted_auth_dashboard_assistant_response"
    return result
...
def run_checks(root: Path, compile_all: bool = True, app_factory: bool = True, secret_gate: bool = True, pytest_gate: bool = True, write_report: bool = True) -> dict[str, Any]:
    ...
    result = {
        ...
        "next_actions": [
            "P3B temizse auth/dashboard/asistan mobil cevap kodu smoke kapisi kurulmus kabul edilebilir.",
