...
DOMAIN_HINTS = {
    ...
    "personnel": ["personnel", "personel", "employee", "profile", "hr", "attendance", "reports"],
    "performance": ["performance", "performans", "task", "score", "period"],
    "support": ["support", "ticket", "destek", "talep"],
    "survey": ["survey", "anket", "question", "answer"],
    "communication": ["message", "communication", "duyuru", "announcement", "notification", "thread"],
    "assistant": ["assistant", "asistan", "ai_agent", "chat", "insight"],
    "settings": ["settings", "ayar", "role", "permission", "menu"],
    "utility": ["health", "ping", "version", "config", "status", "kpi"],
}

HIGH_RISK_HINTS = (
    "password", "set_password", "check_password_hash", "token", "jwt", "session[",
    "commit(", "rollback(", "db.session.add", "db.session.delete", ".delete(",
    "current_user", "login_user", "logout_user", "request.json", "request.form",
    "request.get_json",
)
...
def analyze_function(node: ast.AST, lines: list[str]) -> dict[str, Any]:
    ...
    if not has_route:
        ...
        risk_level = "HIGH"
        reason = "Auth/token/password/write veya POST/PUT/PATCH/DELETE riski var."
    elif count >= 80:
        decision = "DEFER_LARGE_ROUTE"
        risk_level = "HIGH"
        reason = "Büyük route; ayrı özel plan gerekir."
    elif db_read and count > 45:
        decision = "REVIEW_DB_READ_ROUTE"
        risk_level = "MEDIUM"
        reason = "DB okuma içeriyor ve orta/büyük boy."
    elif count <= 30 and not high_risk:
        decision = "LOW_RISK_ROUTE_CANDIDATE"
        risk_level = "MEDIUM"
        reason = "Küçük GET route; yine de domain/smoke kontrolü gerekir."
    elif count <= 50 and not high_risk:
        decision = "REVIEW_LIGHT_ROUTE"
        risk_level = "MEDIUM"
        reason = "Görece küçük GET route; taşıma öncesi domain kontrolü gerekir."
    else:
        decision = "REVIEW_ROUTE"
        risk_level = "MEDIUM"
        reason = "Route var ancak elle değerlendirme gerekir."
...
def main() -> int:
    ...
    print("BYS360_QUALITY_10_10_P11_B8_MOBILE_ROUTES_REMAINING_REFRESH_START")
    ...
    # We now prefer personnel/performance read-only only if they are small and GET-only.
    group_priority = ["performance", "personnel", "dashboard", "support", "survey", "communication", "settings"]
    ...
    recommendation = {
        ...
        "reason": (
            "Kalan küçük GET endpointleri içinde ilk uygun grup budur; yine de write/auth içermediği doğrulanmalıdır."
            if recommended_domain else
            "Güvenli küçük GET adayı kalmadı; mobil routes.py için durup performance_routes veya teknik terim temizliklerine geçmek daha doğru."
        ),
        "next_step": "P11-B9 seçilen düşük riskli GET grubunu bridge ile ayırma." if recommended_domain else "P11-B mobil route split burada durdurulmalı.",
    }
    ...
    report = {
        ...
        "safety_strategy": [
            "Auth, token, password, POST ve commit/rollback içeren endpointlere dokunulmayacak.",
            "Kalan personel/performance endpointleri sadece küçük GET/read ise değerlendirilecek.",
            "Büyük personel listesi, personel oluşturma ve büyük asistan endpointleri ertelenecek.",
            "Her yeni bridge sonrası compileall, audit ve mobil smoke test yapılacak.",
        ],
    }

    out_dir = project_root / "reports" / "quality"
    out_dir.mkdir(parents=True, exist_ok=True)
    json_out = out_dir / "bys360_quality_10_10_p11_b8_mobile_routes_remaining_refresh_v1.json"
    md_out = out_dir / "bys360_quality_10_10_p11_b8_mobile_routes_remaining_refresh_v1.md"
    json_out.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    md = []
    md.append("# BYS360 P11-B8 Mobile Routes Remaining Refresh")
    md.append("")
    ...
    for f in (low_candidates + light_review)[:100]:
        route = ", ".join(r["route"] for r in f["routes"])
        md.append(f"- `{f['name']}` ({f['domain']}) satır {f['start']}-{f['end']} ({f['lines']}), route={route}, decision={f['decision']}")
    md.append("")
    md.append("## Ertelenenler")
    md.append("")
    ...
    print("BYS360_QUALITY_10_10_P11_B8_DECORATOR_SUMMARY")
    for k, v in by_decorator.most_common():
        print(f"{v:>4} | {k}")
    print("BYS360_QUALITY_10_10_P11_B8_DECISION_SUMMARY")
    for k, v in by_decision.most_common():
        print(f"{v:>4} | {k}")
    print("BYS360_QUALITY_10_10_P11_B8_ROUTE_RISK_SUMMARY")
    for k, v in by_risk.most_common():
        print(f"{v:>4} | {k}")
    print("BYS360_QUALITY_10_10_P11_B8_LOW_OR_LIGHT_CANDIDATES")
    for f in (low_candidates + light_review)[:args.limit]:
        route = ",".join(r["route"] for r in f["routes"])
        methods = ",".join(f["methods"]) if f["methods"] else "-"
        print(f"{f['lines']:>4} | {f['domain']} | {f['decision']} | {f['start']}-{f['end']} | {f['name']} | methods={methods} | route={route}")
    print("BYS360_QUALITY_10_10_P11_B8_RECOMMENDATION")
    ...
    print("BYS360_QUALITY_10_10_P11_B8_MOBILE_ROUTES_REMAINING_REFRESH_OK")
    return 0
