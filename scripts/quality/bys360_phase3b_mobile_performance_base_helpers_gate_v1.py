...
PACKAGE = "BYS360_PHASE3B_MOBILE_PERFORMANCE_BASE_HELPERS_GATE_V1"
REPORT_REL = Path("reports/architecture/BYS360_PHASE3B_MOBILE_PERFORMANCE_BASE_HELPERS_GATE_V1_REPORT.json")

ROOT_REL_ROUTE = Path("app/api/mobile/performance_routes.py")
ROOT_REL_SERVICE = Path("app/api/mobile/services/performance_base_helpers.py")
...
def run_checks(root: Path, write_report: bool = True) -> dict[str, Any]:
    ...
    helper_removed_from_route = {
        name: name not in route_functions
        for name in EXPECTED_HELPERS
    }

    route_import_ok = "from app.api.mobile.services.performance_base_helpers import" in route_text

    result: dict[str, Any] = {
        "package": PACKAGE,
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "root": str(root),
        "base_helpers_gate_ok": False,
        ...
        "route_line_reduction_ok": len(route_text.splitlines()) < 1758,
    ...
    result["base_helpers_gate_ok"] = bool(
        result["route_exists"]
...
def main() -> int:
    ...
    return 0 if result["base_helpers_gate_ok"] else 1
