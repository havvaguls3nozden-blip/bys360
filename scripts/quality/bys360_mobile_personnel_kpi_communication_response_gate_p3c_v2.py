...
PACKAGE = "BYS360_MAINTENANCE_SCORE_UPLIFT_P3C_MOBILE_PERSONNEL_KPI_COMMUNICATION_RESPONSE_GATE_V2"
REPORT_REL = Path("reports/architecture/BYS360_MOBILE_PERSONNEL_KPI_COMMUNICATION_RESPONSE_GATE_P3C_V2_REPORT.json")
TEST_FILE = "test_mobile_api_personnel_kpi_communication_response_p3c_v2.py"
...
def runtime_route_map_check(root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        for method, suffix in EXPECTED_ROUTE_SUFFIXES:
            ...
            matches = [r for r in routes if r["rule"].endswith(full_suffix)]
            if not matches:
                missing.append({"method": method, "suffix": suffix, "expected_runtime_suffix": full_suffix})
                continue
            if not any(method in r["methods"] for r in matches):
                method_mismatches.append({
                    "method": method,
                    "suffix": suffix,
                    "runtime_methods": [r["methods"] for r in matches],
                    "runtime_rules": [r["rule"] for r in matches],
                })
...
def ensure_active_test_registered(root: Path) -> dict[str, Any]:
    ...
    text = _read(conftest)
    legacy_tests = [
        "test_mobile_api_personnel_kpi_communication_response_p3c.py",
    ]
    removed_legacy = []
    for legacy in legacy_tests:
        new_text = re.sub(rf'^\s*[\"\']{re.escape(legacy)}[\"\'],\s*$', '', text, flags=re.M)
        if new_text != text:
    ...
    if TEST_FILE in text:
        if removed_legacy:
            ...
            return {"path": str(conftest), "changed": True, "reason": "legacy_removed_already_present", "removed_legacy": removed_legacy}
...
def run_pytest(root: Path) -> dict[str, Any]:
    ...
    if not test_path.exists():
        return {"ok": False, "returncode": 1, "stdout_tail": "", "stderr_tail": f"missing {test_path}", "mode": "pytest_targeted_personnel_kpi_communication_response_p3c_v2"}
    result = run_subprocess([sys.executable, "-m", "pytest", str(test_path.relative_to(root)), "-q"], root)
    result["mode"] = "pytest_targeted_personnel_kpi_communication_response_p3c_v2"
    return result
...
def run_checks(root: Path, compile_all: bool = True, app_factory: bool = True, secret_gate: bool = True, pytest_gate: bool = True, write_report: bool = True) -> dict[str, Any]:
    ...
    compile_targets = [
        root / "scripts" / "quality" / "bys360_mobile_personnel_kpi_communication_response_gate_p3c_v2.py",
    ...
    result: dict[str, Any] = {
        ...
        "pytest_mode": pytest_result.get("mode", "pytest_targeted_personnel_kpi_communication_response_p3c_v2"),
        "inventory": inventory,
