...
PACKAGE = "BYS360_PHASE3C_SCORE_ROUTE_SERVICE_GATE_V1"
REPORT_REL = Path("reports/architecture/BYS360_PHASE3C_SCORE_ROUTE_SERVICE_GATE_V1_REPORT.json")

ROUTE_REL = Path("app/api/mobile/performance_routes.py")
SERVICE_REL = Path("app/api/mobile/services/performance_score_route_services.py")

TARGET_ROUTE_FUNCTIONS = [
    "mobile_performance_task_score_form",
...
def run_checks(root: Path, write_report: bool = True) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "route_import_ok": "performance_score_route_services" in route_text,
        "deps_function_ok": "def _phase3c_score_route_deps(" in route_text,
        ...
        "route_line_reduction_ok": len(route_text.splitlines()) < 1405,
        "score_route_service_gate_ok": False,
    }

    result["score_route_service_gate_ok"] = bool(
        result["route_exists"]
...
def main() -> int:
    ...
    return 0 if result["score_route_service_gate_ok"] else 1
