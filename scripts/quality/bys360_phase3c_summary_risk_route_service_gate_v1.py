...
PACKAGE = "BYS360_PHASE3C_SUMMARY_RISK_ROUTE_SERVICE_GATE_V1"
REPORT_REL = Path("reports/architecture/BYS360_PHASE3C_SUMMARY_RISK_ROUTE_SERVICE_GATE_V1_REPORT.json")

ROUTE_REL = Path("app/api/mobile/performance_routes.py")
SERVICE_REL = Path("app/api/mobile/services/performance_summary_risk_route_services.py")

TARGET_ROUTE_FUNCTIONS = [
    "_bys360_legacy_mobile_performance_full_feature_summary",
    "mobile_performance_publish_preapproval",
    "_bys360_legacy_mobile_performance_risk_analysis_v2852",
]
...
def run_checks(root: Path, write_report: bool = True) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "route_import_ok": "performance_summary_risk_route_services" in route_text,
        "deps_function_ok": "def _phase3c_summary_risk_route_deps(" in route_text,
        "route_globals_bridge_ok": "'__route_globals__': globals()" in route_text and "deps.get('__route_globals__', {})" in service_text,
        ...
        "service_logic_ok": all(token in service_text for token in [
            "_v2852_pending_assignments",
            "_safe_avg_score",
            "_low_score_count",
            "_v2852_items_from_models",
            "Yayın Ön Kontrol",
            "70 Altı Risk",
        ]),
        "route_line_reduction_ok": len(route_text.splitlines()) < 1205,
        "summary_risk_route_service_gate_ok": False,
    }

    result["summary_risk_route_service_gate_ok"] = bool(
        result["route_exists"]
        and result["service_exists"]
        and result["route_decorator_count"] == 22
        and result["route_import_ok"]
        and result["deps_function_ok"]
        and result["route_globals_bridge_ok"]
...
def main() -> int:
    ...
    return 0 if result["summary_risk_route_service_gate_ok"] else 1
