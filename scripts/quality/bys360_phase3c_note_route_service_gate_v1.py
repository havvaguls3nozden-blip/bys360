...
PACKAGE = "BYS360_PHASE3C_NOTE_ROUTE_SERVICE_GATE_V1"
REPORT_REL = Path("reports/architecture/BYS360_PHASE3C_NOTE_ROUTE_SERVICE_GATE_V1_REPORT.json")

ROUTE_REL = Path("app/api/mobile/performance_routes.py")
SERVICE_REL = Path("app/api/mobile/services/performance_note_route_services.py")
...
def run_checks(root: Path, write_report: bool = True) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "route_import_ok": "performance_note_route_services" in route_text,
        "deps_function_ok": "def _phase3c_note_route_deps(" in route_text,
        ...
        "service_logic_ok": all(token in service_text for token in [
            ...
            "_v2853_note_bool",
            "Karne Notu",
            "Dönem içi not kaydedildi",
        ]),
        "route_line_reduction_ok": len(route_text.splitlines()) < 1265,
        "note_route_service_gate_ok": False,
    }

    result["note_route_service_gate_ok"] = bool(
        result["route_exists"]
...
def main() -> int:
    ...
    return 0 if result["note_route_service_gate_ok"] else 1
