...
PACKAGE = "BYS360_MAINTENANCE_SCORE_UPLIFT_P1E_MOBILE_ROUTES_PERSONNEL_KPI_SPLIT"
MARKER = "# BYS360_P1E_MOBILE_ROUTES_PERSONNEL_KPI_SPLIT"
P1D_MARKER = "# BYS360_P1D_MOBILE_ROUTES_COMM_ASSISTANT_SPLIT"
...
def find_kpi_end(text: str, start_index: int) -> int | None:
    # KPI bloğundan sonra route sözleşme/blueprint resolver markerları geliyor. En güvenli son markerı seç.
    end_patterns = [
        r"^# BYS360_MOBILE_V2_8_62_PERSONNEL_ROUTE_MARKER",
        r"^# BYS360_MOBILE_V2_8_64_NOTIFICATIONS_COMPLETION",
        r"^# BYS360_P14F1_MOBILE_BLUEPRINT_RESOLVER_START",
        r"^\s*for\s+_name\s+in\s+\(\"mobile_bp\"",
    ]
    candidates: list[int] = []
    tail = text[start_index + 1 :]
    for pat in end_patterns:
        m = re.search(pat, tail, flags=re.M)
        if m:
            candidates.append(start_index + 1 + m.start())
...
def module_text(domain: str, body: str) -> str:
    ...
    return (
        ...
        "from app.api.mobile.shared import *  # noqa: F401,F403 - BYS360 P1E domain endpoint importu\n\n\n"
        + clean
        + "\n"
    )
...
def ensure_domain_imports(routes_text: str, domains: list[str]) -> str:
    if MARKER in routes_text:
        return routes_text
    imports = [
        f"from app.api.mobile.domains.{domain} import *  # noqa: F401,F403 - P1E domain route registration"
        for domain in domains
    ]
    block = MARKER + "\n" + "\n".join(imports) + "\n"
    # P1D import bloğundan hemen sonra ekle.
    if P1D_MARKER in routes_text:
        lines = routes_text.splitlines()
        insert_at = None
        for idx, line in enumerate(lines):
            if P1D_MARKER in line:
                insert_at = idx + 1
                continue
...
def split_domains(original: str) -> tuple[str, dict[str, str], list[str]]:
    ...
    # Personel tüm liste + personel ekleme/oluşturma bloğu.
    personnel_start = re.search(r"^\s*@mobile_api_bp\.get\([\"']/personnel/all[\"']\)", original, flags=re.M)
    if not personnel_start:
        missing.append("personnel_all_create_start_not_found")
    else:
        personnel_end = re.search(r"^# BYS360_MOBILE_V2_8_53_KPI_TARGET_MANAGEMENT_API", original[personnel_start.start() + 1 :], flags=re.M)
        if not personnel_end:
            missing.append("personnel_all_create_end_not_found")
        else:
            specs.append((personnel_start.start(), personnel_start.start() + 1 + personnel_end.start(), "personnel_write_all"))

    # KPI hedef yönetimi bloğu.
    kpi_start = re.search(r"^# BYS360_MOBILE_V2_8_53_KPI_TARGET_MANAGEMENT_API", original, flags=re.M)
    if not kpi_start:
        missing.append("kpi_target_management_start_not_found")
    else:
        kpi_end = find_kpi_end(original, kpi_start.start())
        if kpi_end is None:
            missing.append("kpi_target_management_end_not_found")
        else:
            specs.append((kpi_start.start(), kpi_end, "kpi_target_management"))

    if missing:
        return original, {}, missing
    specs.sort()
    for idx in range(1, len(specs)):
        if specs[idx][0] < specs[idx - 1][1]:
            return original, {}, ["range_overlap"]

    modules: dict[str, str] = {}
    pieces: list[str] = []
    last = 0
    for start, end, domain in specs:
...
def run_subprocess(root: Path, command: list[str], env_extra: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    try:
        proc = subprocess.run(command, cwd=str(root), text=True, capture_output=True, timeout=90, env=env)
        return {"returncode": proc.returncode, "stdout_tail": proc.stdout[-4000:], "stderr_tail": proc.stderr[-4000:], "ok": proc.returncode == 0}
    except Exception as exc:  # pragma: no cover
...
def run(root: Path, mode: str, run_app_factory_smoke: bool, run_secret_gate: bool) -> dict[str, Any]:
    ...
    if not routes.exists():
        data = {"ok": False, "package": PACKAGE, "generated_at": datetime.now().isoformat(timespec="seconds"), "root": str(root), "mode": mode, "error": "app/api/mobile/routes.py bulunamadı"}
    ...
    already_split = MARKER in original and (domain_dir / "kpi_target_management.py").exists()
    changed: list[str] = []
    missing: list[str] = []

    if mode in {"all", "write"} and not already_split:
        if P1D_MARKER not in original and not (domain_dir / "assistant_chat.py").exists():
            missing.append("p1d_marker_missing")
    ...
    if run_app_factory_smoke:
        app_factory_smoke = run_subprocess(root, [sys.executable, "-c", "from app import create_app; app=create_app(); print('APP_FACTORY_OK')"], env_extra={"APP_ENV": os.environ.get("APP_ENV", "development"), "FLASK_ENV": os.environ.get("FLASK_ENV", "development")})
    ...
    checks = {
        "p1d_marker_present": P1D_MARKER in current_routes or (domain_dir / "assistant_chat.py").exists(),
        "domain_dir_exists": domain_dir.exists(),
        "p1e_domain_files_created": all((domain_dir / f"{name}.py").exists() for name in ["personnel_write_all", "kpi_target_management"]),
        ...
        "routes_py_under_500_lines": routes_py_lines_after < 500,
    ...
    data = {
        ...
        "before": {"routes_py_lines": before_routes_lines, "mobile_contract_route_count": before_route_count},
        "after": {"routes_py_lines": routes_py_lines_after, "mobile_contract_route_count": after_route_count, "domain_files": [str(p.relative_to(root)).replace("\\", "/") for p in domain_files]},
        "checks": checks,
        "compile_results": compile_results,
        "app_factory_smoke": app_factory_smoke,
        "secret_gate": secret_gate,
        "report": str(report_dir / "BYS360_MOBILE_ROUTES_PERSONNEL_KPI_SPLIT_P1E_REPORT.json"),
        "next_actions": [
            "P1F'de mobil API domain smoke testleri auth/dashboard/personnel/kpi/communication/assistant bazında çalıştırılmalıdır.",
            "P1E sonrası routes.py ince facade seviyesine inmiş kabul edilebilir; yeni endpointler ilgili domain dosyasına eklenmelidir.",
            "URL/endpoint sözleşmesi korunmaya devam etmelidir; canlı risk düşük tutulmuştur.",
        ],
    }
    write_outputs(report_dir, docs_dir, data)
    return data
...
def markdown(data: dict[str, Any]) -> str:
    ...
    if data.get("missing_or_blocked"):
        ...
        for item in data.get("missing_or_blocked", []):
            lines.append(f"- `{item}`")
    lines.extend(["", "## Not", "", "Bu faz, P1D sonrası kalan personel tüm liste/ekleme ve KPI hedef yönetimi mobil endpoint bloklarını domain modüllerine taşır. URL/endpoint sözleşmesi korunur; app factory smoke ve secret gate ile doğrulanır."])
    return "\n".join(lines) + "\n"


def write_outputs(report_dir: Path, docs_dir: Path, data: dict[str, Any]) -> None:
    report = report_dir / "BYS360_MOBILE_ROUTES_PERSONNEL_KPI_SPLIT_P1E_REPORT.json"
    report_md = report_dir / "BYS360_MOBILE_ROUTES_PERSONNEL_KPI_SPLIT_P1E_REPORT.md"
    docs_md = docs_dir / "BYS360_MOBILE_ROUTES_PERSONNEL_KPI_SPLIT_P1E_REPORT.md"
...
def main() -> None:
    ...
    print(json.dumps({"ok": data.get("ok"), "package": data.get("package"), "mode": data.get("mode"), "changed_count": data.get("changed_count"), "before": data.get("before"), "after": data.get("after"), "checks": data.get("checks"), "missing_or_blocked": data.get("missing_or_blocked"), "report": data.get("report")}, ensure_ascii=False, indent=2))
    if not data.get("ok"):
        raise SystemExit(1)
