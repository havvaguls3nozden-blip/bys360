...
def ensure_active_scope(root: Path) -> dict[str, Any]:
    """Aktif mimari kapsamına P4B V2 testini ekler, eski P4B testini çıkarır.
    ...
    original = text
    text = text.replace(OLD_ACTIVE_TEST_FILE, ACTIVE_TEST_FILE)
    if ACTIVE_TEST_FILE not in text:
        marker = "# BYS360_P4B_V2_ACTIVE_SCOPE"
        addition = (
            f"\n{marker}\n"
...
def _flask_rule_to_regex(rule: str) -> re.Pattern[str]:
    ...
    # /x/<int:id>/y, /x/<string:name>/y gibi Flask converter parçalarını yakala.
    escaped = re.sub(r"\\<int:[^>]+\\>", r"\\d+", escaped)
    escaped = re.sub(r"\\<string:[^>]+\\>", r"[^/]+", escaped)
    escaped = re.sub(r"\\<path:[^>]+\\>", r".+", escaped)
    escaped = re.sub(r"\\<uuid:[^>]+\\>", r"[0-9a-fA-F-]+", escaped)
    # Converter belirtilmemiş <id> formatı için de güvenli fallback.
...
def pytest_gate(root: Path) -> dict[str, Any]:
    ...
    result["mode"] = "pytest_targeted_mobile_role_boundary_matrix_p4b_v2"
    return result
...
def run_checks(
    ...
    compile_targets = [
        root / "scripts" / "quality" / "bys360_mobile_role_boundary_matrix_gate_p4b_v2.py",
    ...
    result: dict[str, Any] = {
        ...
        "next_actions": [
            "P4B V2 temizse mobil API rol/header/token taklidi sinir matrisi dinamik route eslesmesiyle standart kalite kapisina alinmis kabul edilebilir.",
