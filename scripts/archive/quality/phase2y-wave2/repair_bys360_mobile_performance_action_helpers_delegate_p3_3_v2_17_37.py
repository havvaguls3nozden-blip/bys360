# -*- coding: utf-8 -*-
"""
BYS360 Mobile Performance Action Helpers Delegate P3.3 V2.17.37

This script delegates only small score-action helper functions from
app/api/mobile/performance_routes.py to app/api/mobile/services/performance_task_service.py.
...
VERSION = "V2.17.37"
PATCH_MARKER = "BYS360_MOBILE_PERFORMANCE_ACTION_HELPERS_DELEGATE_P3_3_V2_17_37"
TARGET_REL = Path("app/api/mobile/performance_routes.py")
SERVICE_REL = Path("app/api/mobile/services/performance_task_service.py")
REPORT_JSON_REL = Path("reports/quality/bys360_mobile_performance_action_helpers_delegate_p3_3_v2_17_37_report.json")
REPORT_MD_REL = Path("reports/quality/bys360_mobile_performance_action_helpers_delegate_p3_3_v2_17_37_report.md")
TARGETS = ["_v2837_action_capabilities", "_v2837_find_return_target"]
...
def is_delegated(fn: ast.FunctionDef) -> bool:
    try:
        ...
        body_text = ""
    return "performance_task_service" in body_text or "_bys360_performance_task_service" in body_text
...
def first_def_line(lines: List[str], start_idx: int) -> str:
    # Most target functions are single-line signatures. Keep this strict for safety.
...
def make_wrapper_source(def_line: str, fn: ast.FunctionDef) -> str:
    ...
    return (
        f"{def_line}\n"
        f"{body_indent}from app.api.mobile.services import performance_task_service as _bys360_performance_task_service\n"
        f"{body_indent}return _bys360_performance_task_service.{name}({call_args})\n"
    )
...
def ensure_service_delegates(service: Path, target_names: List[str]) -> bool:
    if service.exists():
        ...
        text = '"""Mobile performance task service delegates."""\n\n'
    changed = False
    additions: List[str] = []
...
def patch_routes(project_root: Path, patch_names: List[str], quarantine_root: Path) -> Dict[str, object]:
    ...
    if not compile_result.get("ok"):
        ...
        return {
            "changed": False,
            "patched_functions": [],
            "backup": str(backup.relative_to(project_root)) if backup.is_relative_to(project_root) else str(backup),
...
def apply(project_root: Path) -> Dict[str, object]:
    ...
    quarantine_root = project_root / "_local_quarantine" / f"bys360_mobile_performance_action_helpers_delegate_p3_3_{stamp}"
    ...
    if not service_compile.get("ok"):
        return {
            ...
            "quarantine_root": str(quarantine_root.relative_to(project_root)),
        }
    ...
    if routes_apply.get("changed") and not h.get("overall_ok"):
        # Restore backup on full health failure.
        backup_str = routes_apply.get("backup") or ""
        backup_path = project_root / backup_str if backup_str else None
...
def write_report(project_root: Path, result: Dict[str, object]) -> None:
    ...
    lines: List[str] = []
    lines.append("# BYS360 Mobile Performance Action Helpers Delegate P3.3 V2.17.37\n")
    lines.append("Bu rapor gercek yazma endpointlerine dokunmadan score-action karar/helper fonksiyonlarinin servis delegasyonu sonucunu gosterir.\n")
    lines.append("## Durum\n")
...
def main() -> int:
    ...
    if args.mode == "all" and apply_data.get("post_apply_health"):
