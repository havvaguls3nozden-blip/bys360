...
PACKAGE = "BYS360_OPS_HARDENING_V6B_PERMISSION_SESSION_AUTH_HOTFIX"
TARGET_HINTS = (
    "auth", "login", "session", "permission", "permissions", "role", "roles",
    "access", "acl", "security", "csrf", "token", "jwt", "user_context",
)
SKIP_DIRS = {
...
def guarded_message(rel: str, line_no: int) -> str:
    ...
    return f'logger.exception("BYS360 V6B guarded exception | file={safe_rel} | line={line_no}")'
...
def apply_to_file(path: Path, root: Path, apply: bool) -> tuple[list[Candidate], list[Change], bool, str | None]:
    ...
    for idx, line in enumerate(lines):
        ...
        candidates.append(Candidate(file=rel, line=idx + 1, risk="P0_PERMISSION_SESSION_AUTH", has_logging_or_raise=has_log))
    ...
    if new_text != original:
        backup_dir = root / "backups" / "ops_hardening_v6b_hotfix" / datetime.now().strftime("%Y%m%d_%H%M%S")
...
def main() -> int:
    ...
    md_path = report_dir / "BYS360_OPS_HARDENING_V6B_PERMISSION_SESSION_AUTH_REPORT.md"
    json_path = report_dir / "BYS360_OPS_HARDENING_V6B_PERMISSION_SESSION_AUTH_REPORT.json"
    ...
    md += ["", "## Not", "V6B HOTFIX geniş `except Exception` ifadelerini daraltmaz; kritik yetki/oturum/kimlik doğrulama hattında sessiz/zayıf bloklara izlenebilir log ekler. Riskli dosyaları atlayarak projeyi bozmaz."]
    md_path.write_text("\n".join(md) + "\n", encoding="utf-8")

    print(f"BYS360_OPS_V6B_REPORT={md_path}")
    print(f"BYS360_OPS_V6B_JSON={json_path}")
