...
NEW_MODULE_REL = "app/api/mobile/communication_v2_read_routes.py"

TARGET_FUNCTIONS = [
    "mobile_b48_communication_v2_threads",
]

IMPORT_SENTINEL = "# BYS360 P11-B7: communication v2 read mobile route bridge"
CALL_LINE = "_register_mobile_communication_v2_read_routes_v1(mobile_bp, globals())"
...
def make_new_module_text(function_sources: list[str]) -> str:
    ...
    parts = [
        ...
        "from __future__ import annotations",
        "",
        "_COMMUNICATION_V2_READ_ROUTE_SOURCE = " + repr(source_blob),
        "",
        "",
...
def apply(project_root: Path, dry_run: bool) -> None:
    ...
    if IMPORT_SENTINEL in text and new_module.exists():
        print("BYS360_QUALITY_10_10_P11_B7_ALREADY_APPLIED")
        return
    ...
    for start, end, name in ranges:
        ...
        output_lines.append(f"# BYS360 P11-B7: {name} communication v2 read route app/api/mobile/communication_v2_read_routes.py modülüne taşındı.")
        current = end + 1
    ...
    print("BYS360_QUALITY_10_10_P11_B7_MOBILE_COMMUNICATION_V2_READ_ROUTE_BRIDGE_START")
    ...
    if dry_run:
        print("BYS360_QUALITY_10_10_P11_B7_DRYRUN_OK")
        return

    stamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_root = project_root / ".quality_backup" / f"p11_b7_mobile_communication_v2_read_route_bridge_{stamp}"
    ...
    report_json = out_dir / "bys360_quality_10_10_p11_b7_mobile_communication_v2_read_route_bridge_v1.json"
    report_md = out_dir / "bys360_quality_10_10_p11_b7_mobile_communication_v2_read_route_bridge_v1.md"
    write_text(report_json, json.dumps(report, ensure_ascii=False, indent=2))

    md = []
    md.append("# BYS360 P11-B7 Mobile Communication V2 Read Route Bridge")
    md.append("")
    ...
    md.append("## Güvenlik")
    md.append("")
    md.append("- Sadece GET communication v2 okuma endpointi taşınmıştır.")
    ...
    print(f"communication_v2_read_route_bridge_report_json={report_json}")
    print(f"communication_v2_read_route_bridge_report_md={report_md}")
    print("BYS360_QUALITY_10_10_P11_B7_MOBILE_COMMUNICATION_V2_READ_ROUTE_BRIDGE_OK")
