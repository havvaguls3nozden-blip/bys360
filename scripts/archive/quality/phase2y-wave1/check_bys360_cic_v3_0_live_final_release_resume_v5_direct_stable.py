...
def main() -> int:
    ...
    if '-ProjectRoot' in sys.argv:
        ...
        if i+1 < len(sys.argv): root=Path(sys.argv[i+1])
    errors=[]
    for rel in REQUIRED:
        if not (root/rel).exists(): errors.append(f'Eksik dosya: {root/rel}')
    if not errors:
        ...
        for needle in ['status_pill','task_status','user_initial','cic_csrf','corporate_information_center_v3_0_final_stable.css']:
            if needle not in base: errors.append(f'base.html içinde eksik ifade: {needle}')
        for needle in [VERSION,'def _cic_phase5_mail_health','def _cic_phase5_readiness','phase5','phase6','release_context']:
            if needle not in svc: errors.append(f'service içinde eksik ifade: {needle}')
        if '\n' in base or "\'" in base: errors.append('base.html içinde kaçışlı newline/tırnak kalıntısı var')
    ...
    if res.returncode != 0: errors.append((res.stdout or '')+(res.stderr or ''))
    if errors:
