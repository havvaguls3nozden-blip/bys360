...
PACKAGE = "BYS360_MAINTENANCE_SCORE_UPLIFT_P5D_ANDROID_RESPONSIVE_RELEASE_SUITE_GATE_V2"
REPORT_REL = Path("reports/architecture/BYS360_ANDROID_RESPONSIVE_RELEASE_SUITE_GATE_P5D_V2_REPORT.json")
...
def _ensure_active_scope_marker(root: Path) -> Dict[str, Any]:
    ...
    marker = "BYS360_ACTIVE_ARCHITECTURE_TEST_P5D_V2_ANDROID_RESPONSIVE_RELEASE_SUITE"
    ...
    if marker in text:
        ...
        text = text.rstrip() + f"\n\n# {marker}: test_android_responsive_release_suite_p5d_v2.py\n"
...
def _pytest_gate(root: Path) -> Dict[str, Any]:
    test_file = root / "tests" / "architecture" / "test_android_responsive_release_suite_p5d_v2.py"
    if not test_file.exists():
        return {"ok": False, "returncode": 127, "stdout_tail": "", "stderr_tail": "P5D pytest file missing", "mode": "pytest_targeted_android_responsive_release_suite_p5d_v2"}
    result = _run([sys.executable, "-m", "pytest", str(test_file.relative_to(root)), "-q"], root)
    result["mode"] = "pytest_targeted_android_responsive_release_suite_p5d_v2"
    return result


def _css_evidence(root: Path, p5b_report: Optional[Dict[str, Any]] = None, p5c_report: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Validate responsive CSS evidence without relying on a single brittle text marker.

    P5B created a real central stylesheet whose header is intentionally human-readable
    ("BYS360 P5B Android Responsive Core Styles") rather than the older synthetic
    marker expected by P5D V1. V2 accepts either the text marker family or the
    authoritative P5B/P5C gate report evidence: css exists, base link exists, required
    marker totals passed and media query counts are at/above threshold.
    """
    p5b_report = p5b_report or {}
    p5c_report = p5c_report or {}
    ...
    p5b_core_report = p5b_report.get("core_css") if isinstance(p5b_report.get("core_css"), dict) else {}
    p5c_targeted_report = p5c_report.get("targeted_css") if isinstance(p5c_report.get("targeted_css"), dict) else {}

    p5b_media_count = p5b_text.count("@media") or int(p5b_report.get("core_css_media_query_count", 0) or 0) or int(p5b_core_report.get("media_query_count", 0) or 0)
    p5c_media_count = p5c_text.count("@media") or int(p5c_report.get("core_css_media_query_count", 0) or 0) or int(p5c_targeted_report.get("media_query_count", 0) or 0)

    p5b_marker_present = (
        "P5B_ANDROID_RESPONSIVE_CORE" in p5b_text
        or "BYS360 P5B Android Responsive Core Styles" in p5b_text
        or bool(p5b_core_report.get("ok"))
        or int(p5b_report.get("core_css_required_marker_total", 0) or 0) >= int(p5b_report.get("core_css_required_marker_count", 10) or 10)
    )
    p5c_marker_present = (
        "P5C_ANDROID_RESPONSIVE_TARGETED" in p5c_text
        or bool(p5c_targeted_report.get("ok"))
        or int(p5c_report.get("targeted_css_required_marker_total", 0) or 0) >= 12
    )
    p5b_linked = P5B_LINK in base_text or "bys360_android_responsive_core_p5b.css" in base_text or bool(p5b_report.get("base_template_link_ok"))
    p5c_linked = P5C_LINK in base_text or "bys360_android_responsive_targeted_p5c.css" in base_text or bool(p5c_report.get("base_template_link_ok"))
    p5b_exists = p5b_css.exists() or bool(p5b_core_report.get("exists"))
    p5c_exists = p5c_css.exists() or bool(p5c_targeted_report.get("exists"))
    ok = bool(
        p5b_exists
        and p5c_exists
        and (base.exists() or bool(p5b_report.get("base_template_link_ok") and p5c_report.get("base_template_link_ok")))
        and p5b_linked
        and p5c_linked
        and p5b_media_count >= MIN_CORE_MEDIA_QUERY_COUNT
        and p5c_media_count >= MIN_TARGETED_MEDIA_QUERY_COUNT
        and p5b_marker_present
        and p5c_marker_present
    )
    return {
        "p5b_css_exists": p5b_exists,
        "p5c_css_exists": p5c_exists,
        "base_template_exists": base.exists(),
        "p5b_linked_in_base": p5b_linked,
        "p5c_linked_in_base": p5c_linked,
        "p5b_media_query_count": p5b_media_count,
        "p5c_media_query_count": p5c_media_count,
        "p5b_marker_present": p5b_marker_present,
        "p5c_marker_present": p5c_marker_present,
        "p5b_report_css_ok": bool(p5b_core_report.get("ok") or p5b_report.get("android_core_css_ok")),
        "p5c_report_css_ok": bool(p5c_targeted_report.get("ok") or p5c_report.get("targeted_responsive_css_ok")),
        "ok": ok,
    }
...
def run_gate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    css = _css_evidence(root, p5b, p5c)
    ...
    if args.compile_all:
        compile_results = _compile_files([
            ...
            root / "tests" / "architecture" / "test_android_responsive_release_suite_p5d_v2.py",
    ...
    output: Dict[str, Any] = {
        ...
        "pytest_mode": pytest.get("mode", "pytest_targeted_android_responsive_release_suite_p5d_v2"),
        ...
        "ci_commands": [
            ...
            "python scripts/quality/bys360_android_responsive_release_suite_gate_p5d_v2.py --root . --compile-all --app-factory --secret-gate --pytest-gate",
        ],
